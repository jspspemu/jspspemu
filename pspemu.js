...
﻿var cpu = require('./core/cpu');
var _gpu = require('./core/gpu');
var _display = require('./core/display');
var _memory = require('./core/memory');
var Memory = _memory.Memory;
var CpuState = cpu.CpuState;
var MipsAssembler = cpu.MipsAssembler;
var ProgramExecutor = cpu.ProgramExecutor;
var SyscallManager = cpu.SyscallManager;
var InstructionCache = cpu.InstructionCache;
...
//# sourceMappingURL=app_test_amd.js.map
...
var Emulator = (function () {
    ...
    Emulator.prototype.startAsync = function () {
        ...
        return this.stopAsync().then(function () {
            ...
            _pspmodules.registerModules(_this.moduleManager);
            _pspmodules.registerSyscalls(_this.syscallManager, _this.moduleManager);
...
exports.registerSyscalls = registerSyscalls;
...
function createNativeFunction(exportId, firmwareVersion, retval, _arguments, _this, internalFunc) {
    var code = '';
    ...
    _arguments.split('/').forEach(function (item) {
        switch (item) {
            case 'EmulatorContext':
                _arguments.push('context');
                break;
            case 'HleThread':
                _arguments.push('state.thread');
                break;
            case 'CpuState':
                _arguments.push('state');
                break;
            case 'Memory':
                _arguments.push('state.memory');
                break;
            case 'string':
                _arguments.push('state.memory.readStringz(' + readGpr32_S() + ')');
                break;
            case 'uint':
                _arguments.push(readGpr32_U() + ' >>> 0');
                break;
            case 'int':
                _arguments.push(readGpr32_S() + ' | 0');
                break;
            case 'ulong':
            case 'long':
                _arguments.push(readGpr64());
                break;
            case 'void*':
                _arguments.push('state.getPointerStream(' + readGpr32_S() + ')');
                break;
            case '':
                break;
            default:
    ...
    code += 'var result = internalFunc.apply(_this, [' + _arguments.join(', ') + ']);';
    ...
    Stream.prototype.writeString = function (str) {
        ...
        try  {
            str.split('').forEach(function (_char) {
                _this.writeUInt8(_char.charCodeAt(0));
            });
        } catch (e) {
...
var StructStringz = (function () {
    ...
    StructStringz.prototype.write = function (stream, value) {
        var items = value.split('').map(function (_char) {
